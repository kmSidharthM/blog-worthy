...
class ApplicationController < ActionController::Base
  ...
  def handle_generic_exception(exception, status = :internal_server_error)
    ...
    error = Rails.env.production? ? "Something went wrong. Please try again later." : exception
  ...
  private
    ...
    def authenticate_user_using_x_auth_token
      ...
      # puts user_email
      # puts request.headers["X-Auth-Email"].presence
      # puts auth_token
      # puts request.headers["X-Auth-Token"].presence
